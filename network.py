...
class Network(object):
    ...
    def update_mini_batch(self, mini_batch, eta):
        ...
        self.weights = [w - eps * (nw + 10 * 2 * w) for w, nw in zip(self.weights, nabla_w)]
    ...
    def backprop(self, x, y):
        ...
        delta = self.cost_derivative(activations[-1], y) + 0.3 * np.linalg.norm(self.weights[-1], 1) / len(self.weights[-1])
        nabla_b[-1] = delta
